...
#ifndef _top_Stage1_real_H_
#define _top_Stage1_real_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "top_Stage1_real_memcore.h"

SC_MODULE(top_Stage1_real) {
...
protected:
    ...
    top_Stage1_real_memcore* top_Stage1_real_memcore_U;
...
public:
    ~top_Stage1_real();
    SC_CTOR(top_Stage1_real) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        top_Stage1_real_memcore_U = new top_Stage1_real_memcore("top_Stage1_real_memcore_U");
        top_Stage1_real_memcore_U->clk      ( clk );
        top_Stage1_real_memcore_U->reset    ( reset );
        top_Stage1_real_memcore_U->ce0      ( i_ce0 );
        top_Stage1_real_memcore_U->we0      ( i_we0 );
        top_Stage1_real_memcore_U->address0 ( memcore_iaddr );
        top_Stage1_real_memcore_U->d0       ( i_d0 );
        top_Stage1_real_memcore_U->q0       ( i_q0 );

        top_Stage1_real_memcore_U->ce1      ( t_ce0 );
        top_Stage1_real_memcore_U->we1      ( t_we0 );
        top_Stage1_real_memcore_U->address1 ( memcore_taddr );
        top_Stage1_real_memcore_U->d1       ( t_d0 );
        top_Stage1_real_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "top_Stage1_real.trace");
            // ports
